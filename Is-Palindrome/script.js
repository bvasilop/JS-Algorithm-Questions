...
'use strict';
function isPalindrome(string) { // takes in a string as a parameter // we ignore any punctuation characters

    string = string.toLowerCase();  // first thing is we turn everything into lowercase using toLowerCase method
                                    // set string = string.lowerCase() invoked

    var charactersArr = string.split(''); // turn string into an array of characters
                                        // use string.split method and pass in empty string

    var validCharacters = 'abcdefghijklmnopqrstuvwxyz'.split(''); // make array of any characters that will be allowed in our string
    ...
    if (validCharacters.indexOf(char) > -1) lettersArr.push(char); // indexOf is an array method that returns the array of the argument passed in
...
if (lettersArr.join('') === lettersArr.reverse().join('')) return true; // return true because it's the same forward as backward
    else return false;
}
